...
HTML5 引入了一系列语义化容器标签，替代无意义的 `<div>`，使页面结构更清晰，利于搜索引擎和浏览器解析。

### 1. 常用语义化容器
- `<div>`：通用块级容器（无语义，用于布局分组）。
- `<span>`：通用行内容器（无语义，用于包裹行内文本）。
- `<header>`：页面或区域的头部（通常包含标题、logo、导航等）。
- `<footer>`：页面或区域的底部（通常包含版权信息、联系方式等）。
- `<main>`：页面的主要内容（一个页面只能有一个 `<main>`）。
- `<article>`：独立的文章内容（如博客文章、新闻报道、评论等）。
- `<section>`：页面中的章节或区块（通常包含标题，与 `<article>` 相比更侧重"部分"）。
- `<aside>`：侧边栏内容（与主内容相关的补充信息，如广告、作者简介等）。
- `<figure>`：包含媒体内容（图片、图表等）及其说明。
- `<figcaption>`：为 `<figure>` 提供标题或说明（嵌套在 `<figure>` 内）。
